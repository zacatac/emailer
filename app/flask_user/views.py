...
def register():
    ...
    if request.method=='POST' and register_form.validate():
        # def icesportsforum_email(email):
        ...
        #     icesportsforum_email(register_form.email.data)
        ...
        # Add User_Roles record using recently create User record
        management = Role.query.filter(Role.name=='management').first()
        employee = Role.query.filter(Role.name=='employee').first()
        if user.email.split("@")[1] == "icesportsforum.com" :            
            role = management
        else:
            role = employee
